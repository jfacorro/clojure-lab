...
(defn right
  ...
  [loc]
    (let [[node {l :l  [r & rnext :as rs] :r :as path}] loc
      ...
      (when (and path rs)
        [(first rs) (into path {:l (conj l node) :r (rest rs)})])))
...
(defn rightmost
  ...
  [loc]
    (let [[node {l :l r :r :as path}] loc]
      (if (and path r)
        [(last r) (assoc path :l (apply conj l node (butlast r)) :r nil)]
        loc)))
...
(defn leftmost
  ...
  [loc]
    (let [[node {l :l r :r :as path}] loc]
      (if (and path (seq l))
        [(first l) (assoc path :l [] :r (concat (rest l) [node] r))]
        loc)))
  ...
  [loc item]
    (let [[node {l :l :as path}] loc]
      (if (nil? path)
        ...
        [node (assoc path :l (conj l item) :changed? true)])))
...
(defn insert-right
  ...
  [loc item]
    (let [[node {r :r :as path}] loc]
      (if (nil? path)
        (throw (new Exception "Insert at top"))
        [node (assoc path :r (cons item r) :changed? true)])))
  ...
  [loc]
    (let [[node {l :l, ppath :ppath, pnodes :pnodes, rs :r, :as path}] loc]
      (if (nil? path)
        ...
        (if (pos? (count l))
          (loop [loc [(peek l) (assoc path :l (pop l) :changed? true)]]
            (if-let [child (and (branch?* loc) (down loc))]
              (recur (rightmost child))
              loc))
